...
this.addNewTheme = function(homeid , theme , themeid){

        return new Promise(function(resolve , reject){
            var tid = (themeid) ? themeid : uuid();
            firebase.database().ref('/lots/'+homeid+'/themes/'+tid).set(theme).then(function(response){
                resolve(response);
            }).catch(function(err){
                reject(err);
            });
